...
## Comments starting with keyint "BBTFNT" have been added at places
...
intsig INOP    'I_NOP'
intsig IHALT   'I_HALT'
intsig IRRMOVL 'I_RRMOVQ'
intsig IIRMOVL 'I_IRMOVQ'
intsig IRMMOVL 'I_RMMOVQ'
intsig IMRMOVL 'I_MRMOVQ'
intsig IOPL    'I_ALU'
intsig IJXX    'I_JMP'
intsig ICALL   'I_CALL'
intsig IRET    'I_RET'
intsig IPUSHL  'I_PUSHQ'
intsig IPOPL   'I_POPQ'
...
intsig RRSP     'REG_RSP'           # Stack Pointer
...
bool instr_valid = f_icode in 
    { INOP, IHALT, IRRMOVL, IIRMOVQ, IRMMOVQ, IMRMOVQ,
      IOPL, IJXX, ICALL, IRET, IPUSHQ, IPOPQ };
...
bool need_regids =
    f_icode in { IRRMOVL, IOPQ, IPUSHQ, IPOPQ, 
             IIRMOVL, IRMMOVQ, IMRMOVQ };

# Does fetched instruction require a constant int?
bool need_valC =
    f_icode in { IIRMOVL, IRMMOVQ, IMRMOVQ, IJXX, ICALL };
...
int d_srcA = [
    D_icode in { IRRMOVL, IRMMOVQ, IOPQ, IPUSHQ  } : D_rA;
...
int d_srcB = [
    D_icode in { IOPL, IRMMOVQ, IMRMOVQ  } : D_rB;
    D_icode in { IPUSHL, IPOPQ, ICALL, IRET } : RRSP;
    1 : RNONE;  # Don't need register
];

## What register should be used as the E destination?
int d_dstE = [
    D_icode in { IRRMOVL, IIRMOVQ, IOPQ} : D_rB;
    D_icode in { IPUSHL, IPOPQ, ICALL, IRET } : RRSP;
...
int mem_addr = [
    M_icode in { IRMMOVL, IPUSHQ, ICALL, IMRMOVQ } : M_valE;
    M_icode in { IPOPL, IRET } : M_valA;
    # Other instructions don't need address
];

## Set read control signal
bool mem_read = M_icode in { IMRMOVL, IPOPQ, IRET };

## Set write control signal
bool mem_write = M_icode in { IRMMOVL, IPUSHQ, ICALL };
...
bool D_bubble =
    ...
    !(E_icode in { IMRMOVL, IPOPQ } && E_dstM in { d_srcA, d_srcB }) &&
      IRET in { D_icode, E_icode, M_icode };
bool E_bubble =
    (
    (E_icode == IJXX && E_ifun != UNCOND && E_valC < E_valA && !e_Cnd) ||
